...
function loadImages(folder) {
    ...
    imageContainer.style.maxHeight = '300px'; // 设置容器最大高度以便滚动
            ...
            img.addEventListener('click', () => {
                ...
                if (!selectedImages.includes(img.src)) {
                    ...
                    imgElement.onload = () => {
                        ...
                        hiddenCanvas.width = imgElement.width; // 设置canvas宽度
                        hiddenCanvas.height = imgElement.height; // 设置canvas高度
...
document.getElementById('select-file').addEventListener('click', () => {
    ...
    label.textContent = document.getElementById('textlabel').innerText;
    const input = document.createElement('input');
    input.type = 'text';
    input.value = localStorage.getItem('filePath') || ''; // 从localStorage获取filePath
    //input.value = filePath;

    // 创建“确认”按钮
    const confirmButton = document.createElement('button');
    confirmButton.textContent = document.getElementById('textConfirm').innerText;
    confirmButton.addEventListener('click', () => {
        filePath = input.value; // 更新 filePath 值
        console.log('Updated filePath in localStorage:', filePath); // 添加调试信息
        localStorage.setItem('filePath', filePath); // 保存到localStorage
        ...
        fileInput.addEventListener('change', (event) => {
            console.log('File input change event triggered'); // 新增调试信息
            const files = event.target.files;
            const newFiles = []; // 用于存储新选择的文件名
            for (let i = 0; i < files.length; i++) {
                const file = files[i];
                newFiles.push(file.name); // 保存文件名到数组
            }
            uploadedFiles = newFiles; // 更新已上传文件数组，清空原有文件名 // 更新已上传文件数组
            localStorage.setItem('uploadedFiles', JSON.stringify(uploadedFiles)); // 保存到localStorage
            console.log('Uploaded files:', uploadedFiles); // 调试输出
        });
        // 触发文件选择对话框
        fileInput.click();
    });
    ...
    // modal.appendChild(exampleButton);
...
// 在页面加载时从localStorage获取uploadedFiles
document.addEventListener('DOMContentLoaded', () => {
    const storedFiles = localStorage.getItem('uploadedFiles');
    document.getElementById('images').style.display = 'block';
    //imageContainer.style.display = 'block'; // 显示 images 容器
    if (storedFiles) {
        uploadedFiles = JSON.parse(storedFiles); // 将字符串解析为数组
        console.log('Loaded uploaded files from localStorage:', uploadedFiles); // 输出调试信息
    }
});
...
function loadImages_myself(folder) {
    ...
    uploadedFiles.forEach(file => {
        const img = document.createElement('img');
        img.src = localStorage.getItem('filePath')  + file;
        img.alt = file;
...
//第二个表情包的
    ...
    part1.style.overflowY = 'scroll'; // 使 part1 可垂直滚动
    part1.style.maxHeight = '60px'; // 设置最大高度以便滚动
    const part2 = document.createElement('div');
    part2.style.overflowY = 'scroll'; // 使 part2 可垂直滚动
    part2.style.maxHeight = '60px'; // 设置最大高度以便滚动
...
drawButton.addEventListener('click', () => {
    ...
    // 创建提示信息弹窗
    const messageBox = document.createElement('div');
    messageBox.textContent = document.getElementById('textElement2').innerText;;
    messageBox.style.position = 'fixed';
    messageBox.style.top = '50%';
    messageBox.style.left = '50%';
    messageBox.style.transform = 'translate(-50%, -50%)';
